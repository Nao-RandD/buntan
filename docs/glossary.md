...
| グループオーナー | ぐるーぷおーなー | グループを作成したユーザー。`group` コレクションの `owner` フィールドで特定。グループ名変更・パスワード変更・グループ削除が可能 |
| フォントサイズ | ふぉんとさいず | アプリ全体の文字サイズ設定。`DynamicTypeSize` の7段階（小〜最大(2倍)）から選択可能 |
...
| Settings | 設定 | フォントサイズ調整・グループ管理が行える設定画面 |
| Owner | オーナー | グループ作成者。グループ管理操作の権限を持つ |
...
| `"fontSizeIndex"` | Int | フォントサイズ選択インデックス（0〜6、デフォルト: 3） |
...
| `group` | `owner` | String | グループ作成者のユーザー名 |
